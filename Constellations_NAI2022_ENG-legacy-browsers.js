...
let expName = 'ConstellationsNAI23';  // from the Builder filename that created this script
let expInfo = {
    'Pseudonym*': '',
    'Age': '',
    'Gender': '',
...
async function updateInfo() {
  ...
  psychoJS.experiment.dataFileName = (("." + "/") + `data/${expInfo["Pseudonym*"]}_${expName}_${expInfo["date"]}`);
...
async function experimentInit() {
  ...
  RSP_ahhaa_2 = new visual.Slider({
    ...
    size: [0.5, 0.06], pos: [0.5, (- 0.05)], ori: 0.0, units: psychoJS.window.units,
    labels: ["Very uncertain", "Very certain"], fontSize: 0.04, ticks: [1, 2, 3, 4, 5, 6, 7],
  ...
  RSP_sudden_2 = new visual.Slider({
    ...
    startValue: undefined,
    size: [0.5, 0.06], pos: [0.5, (- 0.3)], ori: 0.0, units: psychoJS.window.units,
    labels: ["Very gradually", "Very suddenly"], fontSize: 0.04, ticks: [1, 2, 3, 4, 5, 6, 7],
    granularity: 1.0, style: ["RADIO"],
    color: new util.Color('Black'), markerColor: new util.Color('Red'), lineColor: new util.Color('White'), 
  ...
  RSP_pleasant_2 = new visual.Slider({
    ...
    startValue: undefined,
    size: [0.5, 0.06], pos: [0.5, (- 0.55)], ori: 0.0, units: psychoJS.window.units,
    labels: ["Very easy", "Very difficult"], fontSize: 0.04, ticks: [1, 2, 3, 4, 5, 6, 7],
    granularity: 1.0, style: ["RADIO"],
    color: new util.Color('Black'), markerColor: new util.Color('Red'), lineColor: new util.Color('White'), 
...
function intro0RoutineBegin(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('intro0.started', globalClock.getTime());
...
function intro0RoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('intro0.stopped', globalClock.getTime());
...
function intro3RoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    psychoJS.experiment.addData('intro3.started', globalClock.getTime());
...
function intro3RoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData('intro3.stopped', globalClock.getTime());
    // store data for psychoJS.experiment (ExperimentHandler)
...
return async function () {
  ...
  continueRoutine = true; // until we're told otherwise
  // update component parameters for each repeat
  psychoJS.experiment.addData('practice_rsp.started', globalClock.getTime());
...
function practice_rspRoutineEnd(snapshot) {
return async function () {
  ...
  psychoJS.experiment.addData('practice_rsp.stopped', globalClock.getTime());
  // store data for psychoJS.experiment (ExperimentHandler)
